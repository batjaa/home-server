...
5. **All persistent state goes to `/opt/docker/data/<service>/`.** Media files live under `/mnt/storage/Media/`. Both are backed up — except *arr-managed media (`Media/Movies`, `Media/TV`, `Media/Whisparr`) and `usenet/`, which are excluded from the backup (`rsync_extra_opts` in `host_vars/andromon/vars.yml`): the 11 TB backup pool can't hold them, and the *arr catalogs in `_docker_data` can re-download them.
